...
// ---------------");
                ...
                sourceBuilder.Append($@"namespace {mauiProgram.ContainingNamespace.ToDisplayString()};
    ...
    {{
        ...
        {{");
                ...
                foreach (var page in pages)
                {
                    sourceBuilder.AppendLine($"services.AddTransient<{page}>();");
                }

                // add ViewModel registrations
                foreach (var vm in viewModels)
                {
                    sourceBuilder.AppendLine($"services.AddTransient<{vm}>();");
                }

                // add Service registrations
                foreach (var service in services)
                {
                    var ifName = $"I{service}";

                    var abstraction = abstractions.Where(a => a.Name == ifName).FirstOrDefault();

                    if (abstraction is null)
                    {
                        sourceBuilder.AppendLine($"services.AddSingleton<{service}>();");
                    }
                    else
                    {
                        sourceBuilder.AppendLine($"services.AddSingleton<{ifName},{service}>();");
                    }
                }
